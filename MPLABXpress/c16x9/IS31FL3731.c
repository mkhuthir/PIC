...
bool IS31FL3731_Read(uint8_t reg, uint8_t *pData)
{
    ...
    I2C2_MasterWriteTRBBuild(&trb[0], &reg, 1, IS31FL3731_ADDRESS);    // build a Write TRB byte to send Reg. Address (paramters: TRB, data, length, address)
    I2C2_MasterReadTRBBuild(&trb[1], pData, 1, IS31FL3731_ADDRESS);    // build a Read TRB byte to read data from Reg.(paramters: TRB, data, length, address)
...
bool IS31FL3731_Write(uint8_t reg, uint8_t data)
{
    ...
    I2C2_MasterWriteTRBBuild(&trb, buffer, 2, IS31FL3731_ADDRESS);     // build a Write TRB 2 bytes. Address (paramters: TRB, data, length, address)
